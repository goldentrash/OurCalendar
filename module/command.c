...
void add(const char *str)
{
    ...
    enrollTask(mkTask(comp, stDate, enDate, contents));
...
void del(const char *str)
{
    ...
    {
        const *stDate = parameters[2];
        const *enDate = parameters[4];
        if (dateCmpr(stDate, enDate) == -1)
        {
            printSysMsg("유효하지 않은 기간입니다.");
            return;
    ...
    char *uip;
    const char **parameters2[2];
    const char syntax3 = {BooleanStr, LnBrkStr};
    getUsrIpt(uip);
...
int parseParameters(const char *usrIpt, const Str *cmdSyntax, const int syntaxLen, const char **parameters)
{
    int now = 0;
